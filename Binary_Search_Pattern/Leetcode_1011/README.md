...
**IDEA** :  
    ...
    - Guess the ship capacity and check if it works.
    - If a ship capacity C is sufficient to ship all packages within D days, then any capacity greater than C will also work.
...
- Apply binary search over a range [low, high], where
        ...
        high = accumulate(weights)

- While low <= high:

      => Compute mid  and treat it ship capacity.
      => if days_needed(mid)  <= days  :  Store mid as a possible answer and look further left to get minimum capacity
      => if days_needed(mid)  > days  :  discard that left half and move to right half
